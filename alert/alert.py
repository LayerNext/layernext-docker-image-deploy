...
def display_memory_usage(host_name):
    ...
    details = (
        ...
        + str(mem_utilization)
        + " %\n\nTotal memory: "
        + str(round(mem_stats.total / (1024**3), 2))
        + " GB\nUsed memory: "
        + str(round(mem_stats.used / (1024**3), 2))
        + " GB\nAvailable memory: "
        + str(round(mem_stats.available / (1024**3), 2))
        + " GB"
    )
...
if load5 >= LOAD_AVERAGE_THRESHOLD:
    print(
        "\nMaximum load average usage threshold: "
        + str(LOAD_AVERAGE_THRESHOLD)
        + " reached"
    )
...
if memory_percentage >= MEMORY_THRESHOLD_PERCENTAGE:
    print(
        ...
        + str(MEMORY_THRESHOLD_PERCENTAGE)
        + "% reached"
    )
