...
def dump_mongdb():
    ...
    try:
        ...
        send_email(
            subject=f"MongoDB Dump Failed for {COMPANY} ({OUTPUT_DIRECTORY}) - {date_string}",
            body=f"An error occurred while creating the MongoDB dump for {OUTPUT_DIRECTORY} on {date_string}. Error details: {str(e)}",
        )
